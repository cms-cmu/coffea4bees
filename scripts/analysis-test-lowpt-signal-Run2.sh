...
JOB="analysis_test_lowpt_signal_Run2"
OUTPUT_DIR=$OUTPUT_BASE_DIR/$JOB
create_output_directory "$OUTPUT_DIR"

# Modify the config file
display_section_header "Modifying config"
JOB_CONFIG=$OUTPUT_DIR/HH4b_lowpt_signal.yml
sed -e 's|apply_FvT: .*|apply_FvT: false|' \
    -e 's|blind:.*|blind: false|' \
    -e 's|plot_ttbar_with_weights: true|plot_ttbar_with_weights: false|' \
    coffea4bees/analysis/metadata/HH4b_lowpt_2024_v2.yml > $JOB_CONFIG
...
bash coffea4bees/scripts/run-analysis-processor.sh \
    ...
    --output-subdir "$JOB" \
    --config $JOB_CONFIG \
    --dataset-metadata "coffea4bees/metadata/datasets_HH4b_Run2/2024_v2/" \
