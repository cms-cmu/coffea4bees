...
JOB="weights_trigger_friendtree_Run3_2024"
...
JOB_CONFIG=$OUTPUT_DIR/trigger_weights_Run3_2024.yml
sed -e "s|make_.*|make_classifier_input: $OUTPUT_DIR|" \
    coffea4bees/analysis/metadata/trigger_weights_Run3_2024.yml > $JOB_CONFIG
cat $JOB_CONFIG; echo
...
bash coffea4bees/scripts/run-analysis-processor.sh \
    ...
    --year "2024" \
    --output-filename "test_trigger_weights_friends_Run3_2024.json" \
    --output-subdir $JOB \
    --config $JOB_CONFIG \
#    --no-test
