...
JOB_CONFIG=$OUTPUT_DIR/trigger_weights_Run3.yml
sed -e "s|make_.*|make_classifier_input: $OUTPUT_DIR|" \
    coffea4bees/analysis/metadata/trigger_weights_Run3.yml > $JOB_CONFIG
cat $JOB_CONFIG; echo
...
bash coffea4bees/scripts/run-analysis-processor.sh \
    ...
    --output-filename "test_trigger_weights_friends.json" \
    --output-subdir $JOB \
    --config $JOB_CONFIG \
#    --no-test
