...
rule create_metadata:
    input: f"{config['output_path']}JCM_2024_v2/jetCombinatoricModel_SB_2024_v2.yml"
    output: f"{config['output_path']}HH4b_nominalJCM.yml"
    shell:
        """
        echo "Modifying metadata file to include new JCM"
        sed -e 's|  JCM_file.*|  JCM_file: {input}|' -e 's|JCM_lowpt_file|#JCM_lowpt_file|' -e 's|apply_JCM_lowpt: true|apply_JCM_lowpt: false|' coffea4bees/analysis/metadata/HH4b_lowpt_2024_v2.yml > {output}
        cat {output}
        """

use rule analysis_processor from analysis as analysis_nojcm_lowpt with:
    input: f"{config['output_path']}HH4b_nominalJCM.yml"
    output: f"{config['output_path']}histAll_nominalJCM_noJCMlowpt_{{dataset}}.coffea"
    params:
        datasets = "{dataset}",
        years = config['year'],
        ...
        username = username
    log: f"{config['output_path']}logs/analysis_nojcm_lowpt_{{dataset}}.log"

use rule merging_coffea_files from analysis as merging_noJCMlowpt_files with:
    input: expand(f"{config['output_path']}histAll_nominalJCM_noJCMlowpt_{{dataset}}.coffea", dataset=config['dataset'])
    output: f"{config['output_path']}histAll_nominalJCM_noJCMlowpt.coffea"
    params:
        run_performance = False
    log: f"{config['output_path']}logs/merging_noJCMlowpt_files.log" 

use rule make_new_JCM as make_new_JCM_lowpt with:
    input: f"{config['output_path']}histAll_nominalJCM_noJCMlowpt.coffea"
    output: f"{config['output_path']}JCM_lowpt_2024_v2/jetCombinatoricModel_SB_2024_v2.yml"
    params:
        extra_arguments = "--lowpt -m coffea4bees/plots/metadata/plotsJCM_lowpt.yml",
        tag = "2024_v2",
...
rule create_metadata_lowpt:
    ...
    output: f"{config['output_path']}HH4b_nominalJCM_wlowptJCM.yml"
    params:
        jcm = f"{config['output_path']}JCM_2024_v2/jetCombinatoricModel_SB_2024_v2.yml"
    shell:
        """
        echo "Modifying metadata file to include new JCM"
        sed -e 's|  JCM_file.*|  JCM_file: {params.jcm}|' -e 's|JCM_lowpt_file.*|JCM_lowpt_file: {input}|' coffea4bees/analysis/metadata/HH4b_lowpt_2024_v2.yml > {output}
        cat {output}
        """

use rule analysis_processor from analysis as analysis_lowpt with:
    input: f"{config['output_path']}HH4b_nominalJCM_wlowptJCM.yml"
