...
rule analysis_test_mixed:
    output: [f"{output_dir}/analysis_test_mixed/testMixedData.json", f"{output_dir}/analysis_test_mixed/testMixedData.coffea"]
    log: f"{output_dir}/analysis_test_mixed.log"
    container: analysis_container
...
rule weights_trigger_analysis:
    input: f"{output_dir}/weights_trigger_friendtree/trigger_weights_friends.json"
    output: f"{output_dir}/weights_trigger_analysis/test_trigWeight.coffea"
    log: f"{output_dir}/weights_trigger_analysis.log"
    container: analysis_container
...
rule analysis_mixed_cutflow:    
    input: f"{output_dir}/analysis_test_mixed/testMixedData.coffea"
...
rule weights_trigger_cutflow:
    input: f"{output_dir}/weights_trigger_analysis/test_trigWeight.coffea"
...
rule workflow_validation:
    ...
    shell:
        """
        pixi run snakemake -s coffea4bees/workflows/Snakefile_full_analysis.smk --configfile coffea4bees/workflows/config/nominal_run2.yml -n 2>&1 | tee -a {log}
        pixi run snakemake -s coffea4bees/workflows/Snakefile_full_analysis.smk --configfile coffea4bees/workflows/config/lowpt_run2.yml -n 2>&1 | tee -a {log}
        pixi run snakemake -s coffea4bees/workflows/Snakefile_full_analysis.smk --configfile coffea4bees/workflows/config/nominal_run3.yml -n 2>&1 | tee -a {log}
        """
