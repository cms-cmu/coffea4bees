...
class mixingTestCase(unittest.TestCase):
    ...
    def test_transverse_thrust_awkward(self):
        # -------------------------
        # Minimal usage examples
        # -------------------------
        # Example A: Directly on a NanoEvents collection (e.g., jets)
        # res = transverse_thrust_awkward(events.Jet)
        # print(ak.to_list(res[:3]))

        # Example B: Build Momentum4D from components and run
        px = ak.Array([[30., -20., 10., -5.], [50.]])
        py = ak.Array([[10.,  15., -8.,  2.], [ 0.]])
        pz = ak.Array([[ 5.,  -2.,  3., -1.], [ 1.]])
        E  = ak.Array([[40.,  30., 20., 10.], [60.]])
        p4 = ak.zip({"px": px, "py": py, "pz": pz, "E": E}, with_name="Momentum4D")
        start = time.perf_counter()
        res = transverse_thrust_awkward(p4, n_steps=720, refine_rounds=0)
        end = time.perf_counter()
        elapsed_time_matrix_python = (end - start)
        print(ak.to_list(res))
        print(f"\nElapsed time loops no refine Python = {elapsed_time_matrix_python}s")

        start = time.perf_counter()
        res = transverse_thrust_awkward(p4, n_steps=720, refine_rounds=2)
        end = time.perf_counter()
        elapsed_time_matrix_python = (end - start)
        print(ak.to_list(res))
        print(f"\nElapsed time loops no w/refine Python = {elapsed_time_matrix_python}s")


        start = time.perf_counter()
        res = transverse_thrust_awkward_fast(p4, n_steps=720, refine_rounds=2)
        end = time.perf_counter()
        elapsed_time_matrix_python = (end - start)
        print(ak.to_list(res))
        ...
        start = time.perf_counter()
        res = transverse_thrust_awkward(self.input_jets_all, n_steps=720, refine_rounds=2)
        end = time.perf_counter()
        elapsed_time_matrix_python = (end - start)
        print(ak.to_list(res))
        print(f"\nElapsed time loops no w/refine Python = {elapsed_time_matrix_python}s")
        ...
        start = time.perf_counter()
        res = transverse_thrust_awkward_fast(self.input_jets_all, n_steps=720, refine_rounds=2)
        end = time.perf_counter()
        elapsed_time_matrix_python = (end - start)
        print(ak.to_list(res))
        print(f"\nElapsed time fast no w/refine Python = {elapsed_time_matrix_python}s")
